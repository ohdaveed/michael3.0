...
- **`tests/e2e/`** — 11 Playwright specs (accessibility, analytics, faq, fixtures, funding-checklist, navigation, onboarding-tour, probate-calculator, quiz, send-a-message, sticky-cta) plus `tests/e2e/fixtures.js` helpers and a `fixtures/` directory. One `chromium` project, `baseURL http://localhost:5173`, trace on first retry, screenshots on failure. Under CI: `forbidOnly`, 2 retries, 2 workers.
  - `accessibility.spec.js` runs axe-core over all 11 built pages with **no rules disabled**. It emulates `reducedMotion` in a `beforeEach`, which is load-bearing for coverage rather than just stability: `js/scroll-reveal.js` holds every `.animate` element at `opacity: 0` until it scrolls into view, and axe skips invisible elements, so without it most of each page below the fold is never scanned and whatever is mid-fade is sampled at a blended colour that never settles. It must be `page.emulateMedia()` — `test.use({ reducedMotion })` leaves the media query matching `no-preference` under Playwright 1.61.1, silently scanning far less than it appears to. A guard test asserts the emulation actually took hold, because the failure mode is a shrunken scan that still reports green.
